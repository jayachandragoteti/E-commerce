...
if(isset($_GET['coustomer_email_verification'])){
    ...
    $select_users=mysqli_query($connect,"SELECT `verified` FROM `users` WHERE `otp` = '$OTP' AND `verified` = '0' LIMIT 1");
    if (mysqli_num_rows($select_users) == 1) {
        $update_status=mysqli_query($connect,"UPDATE `users` SET  `verified` = '1' WHERE `otp` = '$OTP'");
        if ($update_status) {
